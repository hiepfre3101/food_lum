        ...
        <div class="info-clident">
            <form action="?ctr=status-Order&&idOrder=<?=$_GET['idOrder']?>" method="post">
                ...
                <div class="btn-status">
                    <input type="radio" value="1" hidden <?=(getOneDataOrder($_GET['idOrder'])['status']==1)?"checked":""?> name="status" id="check1">
                    <label for="check1">Đơn Hàng Mới</label>
                    <input type="radio" value="2" <?=(getOneDataOrder($_GET['idOrder'])['status']==2)?"checked":""?> hidden name="status" id="check2">
